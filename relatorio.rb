...
pdf.add_text "Inicialmente, o sistema de construção escolhido pro código foi o meson, com objetivo de implementar uma versão do programa opcional com interface gráfica feita em Raylib. Porém, visando melhor portabilidade e tendo em vista o pequeno porte do projeto, os integrantes decidiram se reter a compilação manual apenas com g++ ou clang (comando clang++)."
...
pdf.add_text "Por fim, o relatório da atividade avaliativa foi escrito dentro do repositório Git, para ser montável assim como o resto do programa. Ainda sobraram arquivos redudantes dos subprojetos (raylib e implementação em C) no git, para fins de análise. O relatório foi escrito na linguagem de programação Ruby, seu código junto com a versão já em pdf disponíveis no repositório. A implementação do processo de montagem foi implementação própria do projeto. O código autoral inteiro no repositório foi submetida à licença Unlicense, tornando o repositório, efetivamente, domínio público."
